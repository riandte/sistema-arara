...
import { Pendencia, StatusPendencia, TipoPendencia, PrioridadePendencia, OrigemPendencia } from '../types';
import { AuthContext } from '../auth/authContext';
import { assertPermission, hasPermission, ForbiddenError } from '../auth/permissions';
import { prisma } from '@/lib/db';
import { PendencyStatus, PendencyType, Priority, OriginType, ConclusionType } from '@prisma/client';
...
export const PendenciaService = {
  ...
  async atualizar(id: string, dados: Partial<Pendencia>, context: AuthContext): Promise<Pendencia | null> {
    try {
      ...
      } else if (dados.status && dados.status !== 'CONCLUIDO' && dados.status !== 'ENCERRADA_SEM_CONCLUSAO') {
          // Reabrindo
